...
Follow these steps to create and configure a minimal‑permission GitHub App that will authenticate the release workflow. This should be done for you by the NHS GitHub Admins. You can perform this setup yourself for testing..
...
```bash
USER_NAME="Dan Stefaniuk" # must match GIT_SIGN_BOT_NAME and be passed to `cycjimmy/semantic-release-action@v4`
USER_EMAIL="daniel.stefaniuk@gmail.com" # must match GIT_SIGN_BOT_EMAIL and be passed to `cycjimmy/semantic-release-action@v4`
file=$USER_EMAIL
mkdir -p "$HOME/.gnupg"
